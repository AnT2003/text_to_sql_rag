...
2. **Định danh đầy đủ:** Luôn sử dụng tên bảng dạng `dataset.table` (Full Qualified Name).
3. **Mapping Logic:**
   ...
   - Xác định chính xác giá trị ID tương ứng với mô tả user hỏi. Tìm trong code SQL của routine (mệnh đề `CASE WHEN`). 
   - KHÔNG ĐƯỢC ĐOÁN MÒ.
4. **Viết SQL:**
   - Dùng đúng tên bảng (có backtick `).
...
2. Giải thích ngắn gọn logic truy vấn.
"""
